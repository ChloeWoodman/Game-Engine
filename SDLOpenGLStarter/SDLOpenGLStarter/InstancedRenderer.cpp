...
namespace GE {
	...
	void InstancedRenderer::setInstanceData(const std::vector<InstancePosRotScaleLife>& instances) {
	...
	void InstancedRenderer::drawInstanced(Camera* cam, Model* m, Textures* tex) {
	...
	void InstancedRenderer::updateInstanced(const std::vector<InstancePosRotScaleLife>& instances)
	{
		std::vector<glm::mat4>instance_matrices;
		// Process the location, rotation and scales from instances vector
		// calculating the corresponding transformation matrix and insert
